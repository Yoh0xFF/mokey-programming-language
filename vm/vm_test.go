...
func TestHashLiterals(t *testing.T) {
	tests := []vmTestCase{
		...
		},
		{
			"{1: 2, 2: 3}",
			map[object.HashKey]int64{
				(&object.IntObject{Value: 1}).HashKey(): 2,
				(&object.IntObject{Value: 2}).HashKey(): 3,
			},
		},
		{
			"{1 + 1: 2 * 2, 3 + 3: 4 * 4}",
			map[object.HashKey]int64{
				(&object.IntObject{Value: 2}).HashKey(): 4,
				(&object.IntObject{Value: 6}).HashKey(): 16,
			},
		},
	}
...
func testExpectedObject(
	...
	case map[object.HashKey]int64:
		hash, ok := actual.(*object.HashObject)
		if !ok {
...
func testIntegerObject(expected int64, actual object.Object) error {
	result, ok := actual.(*object.IntObject)
	if !ok {
...
func testBooleanObject(expected bool, actual object.Object) error {
	result, ok := actual.(*object.BoolObject)
	if !ok {
